function [db1SS_D, db2SS_D_Rand] = ClusterSumSquaredDist(db2Data, inNCluMax, inNIter)
...
db1SS_D         = nan(1, inNCluMax - 1); 
db2SS_D_Rand    = nan(inNIter, inNCluMax - 1);
...
for iItr = 1:inNIter
    ...
    for iNCl = 2:inNCluMax
        ...
        db2SS_D_Rand(iItr, iNCl - 1) = SS_Dist(db2DataRnd, iNCl);
    end
end

% Computes the observation probabilities
for iNCl = 2:inNCluMax
    db1SS_D(iNCl - 1) = SS_Dist(db2Data, iNCl);
end

function db1Score = CBASS_U_MultiChannelTemplateMatching(db2Signal, db2Template, blCenter, blNormalize)
...
if blCenter, db2Template = db2Template - mean(db2Template(:)); end
db2Template = db2Template' ./ norm(db2Template(:));

% Computes the centering offset if needed
if blCenter
    dbXCntr = sum(db2Signal(:, 1:end - inNSmpTmp));
    for iSmp = 2:inNSmpTmp
        dbXCntr = dbXCntr + sum(db2Signal(:, iSmp:end - inNSmpTmp + iSmp - 1));
    end
    dbXCntr = dbXCntr./numel(db2Template);
else
    dbXCntr = 0;
end

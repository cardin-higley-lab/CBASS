...
def EnrichmentScore(db2Data, bl1State, bl1Baseline, inNClu, dbSigThrs, inNItr, sOPTION):
    ...
    # if verbose: print('inNObs: ',inNObs)
...
#     if isrow(bl1Baseline), bl1Baseline = bl1Baseline'; end
    ...
    if sOPTION.blZScore: db2Data = stats.zscore(db2Data, axis=0, ddof=1)

    # Initializes the count vector
    in1SigCount = np.zeros([inNObs, 1]);
    # if verbose: print('in1SigCount.shape: ',in1SigCount.shape)
...
#      4. Adds a count for each point situated in an enriched region
    for iItr in range(inNItr):
        start_binomialTest = time.time()
        # Select intialization centroids
        db2Cntr = DrawCentroids(db2Data, inNClu, verbose);
        ...
        # if verbose: 
        #     print('db2D: ',db2D)
        #     print('db2D.shape: ',db2D.shape)
        in1Clu    = np.argmin(db2D, axis=1)
        # if verbose: 
        #     print('in1Clu: ',in1Clu)
        #     print('in1Clu.shape: ',in1Clu.shape)
        ...
        for iClu in range(inNClu):
            ...
            # if verbose: 
            #     print('bl1Clu: ',bl1Clu)
            #     print('bl1Clu.shape: ',bl1Clu.shape)
            inNObs = np.sum(bl1Clu) #inNObs = len(bl1Clu)
#             if verbose: 
#                 print('inNObs: ',inNObs)
# #                 print('inNObs.shape: ',inNObs.shape)
#                 print('bl1State[bl1Clu]: ',bl1State[bl1Clu])
#                 print('np.sum(bl1State[bl1Clu]): ',np.sum(bl1State[bl1Clu]))
#                 print('np.sum(bl1State[bl1Clu] | bl1Baseline[bl1Clu]: ',np.sum(bl1State[bl1Clu] | bl1Baseline[bl1Clu]))
#             dbRate = len(np.where(bl1State[bl1Clu]==True)[0])/len(np.where(bl1State[bl1Clu]|bl1Baseline[bl1Clu]==True)[0]) 
            dbRate = np.sum(bl1State[bl1Clu])/np.sum(bl1State[bl1Clu] | bl1Baseline[bl1Clu])
            # if verbose: 
                # print('dbRate: ',dbRate)
            
            dbXPVal = BinomialTest(inNObs, dbRate, dbRate_All,verbose)
            # if verbose: print('dbXPVal: ',dbXPVal)
                
            if (dbRate > dbRate_All) &  (dbXPVal < dbSigThrs):
                in1SigCount[bl1Clu] = in1SigCount[bl1Clu] + 1
        # if verbose: print('-- Enrichment computation for iter {}/{}: {}'.format(iItr,inNItr, time.time()-start_binomialTest))
...
def DrawCentroids(db2Data, inNClu, verbose):
    ...
    # if verbose: print('in1Idx: ',in1Idx)
    db2Cntr = db2Data[in1Idx, :]
    # if verbose: print('db2Cntr: ',db2Cntr)

    return db2Cntr

# Function 2 - Performs a Binomial test for one sample and a test rate
def BinomialTest(inXNObs, dbXRate, dbXRate_Tot,verbose):
#     dbXPVal = 2 .* (1 - normcdf(abs(dbXRate - dbXRate_Tot)./sqrt(dbXRate_Tot .* (1 - dbXRate_Tot) ./ inXNObs)));

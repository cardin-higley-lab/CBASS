...
.. math:: T = d_{CC} / \sqrt{1 / N_{low} + 1 / N_{up}}

where :math:`d_{CC}` is the Mahalanobis distance between centroids above and under threshold and :math:`N_{low}` and :math:`N_{up}` are the number of events under and above threshold. :math:`T` can be thought of as an analog of the student *t* statistics in multidimensional spaces. Here, searches of the value of :math:`s(i)` maximizing :math:`T` are implemented using the simplex method (Matlab function ``fminsearch`` or the Scipy function ``fmin``). 
